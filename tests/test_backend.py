...
def test_interact_endpoint_success(mock_groq_class):
    ...
    mock_completion.choices = [
        MagicMock(
            message=MagicMock(
                content='{"persona": "Sleek Executive", "hero_title": "Less is More", "hero_subtitle": "Subtle premium styles for modern setups.", "recommended_product_id": "prod_premium_minimal"}'
            )
        )
    ]
    ...
    response = client.post(
        "/api/interact",
        json={"session_id": "session-xyz", "interaction_type": "swipe", "value": "Minimalist"},
    )

    assert response.status_code == 200
    data = response.json()
    assert data["session_id"] == "session-xyz"
    assert data["persona"] == "Sleek Executive"
    assert data["hero_title"] == "Less is More"
    assert data["recommended_product_id"] == "prod_premium_minimal"
    assert len(data["graph_snapshot"]["nodes"]) == 2
...
def test_interact_endpoint_groq_failure_fallback(mock_groq_class):
    mock_instance = MagicMock()
    mock_groq_class.return_value = mock_instance
    mock_instance.chat.completions.create.side_effect = Exception("Groq rate limit error")

    response = client.post(
        "/api/interact",
        json={"session_id": "session-error-test", "interaction_type": "swipe", "value": "Bold & Vibrant"},
    )

    assert response.status_code == 200
    data = response.json()
    assert data["persona"] == DEFAULT_PERSONA["persona"]
    assert data["hero_title"] == DEFAULT_PERSONA["hero_title"]
    assert data["recommended_product_id"] == DEFAULT_PERSONA["recommended_product_id"]
    assert len(data["graph_snapshot"]["nodes"]) == 2
...
def test_interact_endpoint_invalid_json_fallback(mock_groq_class):
    ...
    mock_completion = MagicMock()
    mock_completion.choices = [
        MagicMock(message=MagicMock(content="Here is the output: this is not valid JSON!"))
    ]
    mock_instance.chat.completions.create.return_value = mock_completion

    response = client.post(
        "/api/interact",
        json={"session_id": "session-bad-json", "interaction_type": "select", "value": "Eco-Friendly"},
    )
